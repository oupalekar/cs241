...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    addr32 top_20_bytes = virtual_address & 0xFFFFF000;
    page_table_entry * pte = tlb_get_pte(&this->tlb, top_20_bytes);
    ...
    if(pte == NULL) {
        ...
        addr32 base_address = (virtual_address) >> (VIRTUAL_ADDR_SPACE - NUM_OFFSET_BITS);
        page_directory_entry *pde = &pd->entries[base_address];
        ...
        addr32 pt_address = (virtual_address & 0x003FF000) >> NUM_OFFSET_BITS;
        pte = &(pt->entries[pt_address]);
    }
    ...
    pte->accessed = 1;
    void * f = (void*)get_system_pointer_from_pte(pte);
    memcpy(buffer, f + (virtual_address & 0xFFF), num_bytes);

    tlb_add_pte(&this->tlb, top_20_bytes, pte);
}
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    addr32 x = virtual_address & 0xFFFFF000;
    page_table_entry * pte = tlb_get_pte(&this->tlb, x);
    ...
    pte->dirty = 1;
    void * f = (void*)get_system_pointer_from_pte(pte);
    memcpy(f + (virtual_address & 0xFFF), buffer, num_bytes);

    tlb_add_pte(&this->tlb, x, pte);
}
